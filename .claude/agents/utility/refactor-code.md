---
name: refactor-code
description: "Senior software architect specializing in safe, incremental refactoring analysis"
---
...
def analyze_complexity(file_path): # Use ast module for exact metrics # Calculate cyclomatic complexity per function # Measure nesting depth precisely # Count decision points, loops, conditions # Generate maintainability index
...
cp target*file.py backup_temp/target_file_original*$(date +%Y-%m-%d\_%H%M%S).py

# For multiple files (adjust file pattern as needed)

find . -name "_.{py,js,java,ts,go,rb}" -path "./src/_" -exec cp {} backup*temp/{}\_original*$(date +%Y-%m-%d\_%H%M%S) \;
...
def complex_function(data, config, user, session, cache, logger): # 125 lines of nested logic
if data:
for item in data:
if item.type == 'A': # 30 lines of processing
...
def **init**(self, config, user, session, cache, logger):
self.config = config # ...
