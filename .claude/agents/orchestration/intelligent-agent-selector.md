...
description: "Intelligent Agent Selector that analyzes project context and task requirements to select optimal agents. Combines sophisticated context analysis with historical performance data for evidence-based agent recommendations."
model: opus
tools: context-analyzer, performance-tracker, recommendation-engine
auto_execute: true
auto_confirm: true
strict: true
mcp:
...
def **init**(self):
self.technology_patterns = { # Frontend patterns
'react': {
...
def **init**(self):
self.agent_profiles = { # Universal Specialists
...
def **init**(self, context_analyzer, capability_db, performance_selector):
