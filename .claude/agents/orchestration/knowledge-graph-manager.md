...
description: "Knowledge Graph Manager serving as the central nervous system for collaborative AI projects. Dynamically manages project context, agent activities, and information flow across complex multi-agent workflows with real-time synchronization."
model: opus
tools: graph-builder, context-synchronizer, knowledge-integrator
auto_execute: true
auto_confirm: true
strict: true
mcp:
...
def **init**(self, knowledge_graph_manager):
...
def **init**(self, mcp_client, knowledge_graph):
self.mcp = mcp_client
...
def **init**(self, knowledge_graph, active_agents):
self.kg = knowledge_graph
