...
def analyze_svelte_component(file_path):
    ...
    if re.search(r"\$:\s*\w+\s*=.*\.(filter|map|reduce)\s*\(", content):
        metrics["issues"].append(
            "Array operations in reactive statements - consider $derived"
        )

    # Memory leak patterns
    if re.search(r"addEventListener|setInterval|setTimeout", content) and not re.search(
        r"removeEventListener|clearInterval|clearTimeout|onDestroy", content
    ):
        metrics["issues"].append(
            "Event listeners or timers without cleanup - add cleanup in onDestroy"
        )
    ...
    if global_styles > 5:
        metrics["issues"].append(
            f"Many global styles ({global_styles}) - prefer scoped styles"
        )
    ...
    if inline_styles > 10:
        metrics["issues"].append(
            f"Many inline styles ({inline_styles}) - consider CSS classes"
        )
...
def generate_report(metrics, file_path):
    ...
    if metrics["lines"] > THRESHOLDS["max_lines"]:
        warnings.append(
            f"Component is large ({metrics['lines']} lines) - consider splitting"
        )
    ...
    if metrics["state_vars"] > THRESHOLDS["max_state_vars"]:
        warnings.append(
            f"Many state variables ({metrics['state_vars']}) - consider consolidation"
        )
        metrics["suggestions"].append("Group related state into objects")

    if metrics["effects"] > THRESHOLDS["max_effects"]:
        warnings.append(
            f"Many effects ({metrics['effects']}) - review for optimization"
        )
        metrics["suggestions"].append(
            "Consider if some effects can be converted to $derived"
        )

    if metrics["each_blocks"] > THRESHOLDS["max_each_blocks"]:
        warnings.append(
            f"Multiple each blocks ({metrics['each_blocks']}) - check render performance"
        )
        metrics["suggestions"].append("Consider virtualization for large lists")

    if metrics["nested_each"] >= THRESHOLDS["max_nested_each"]:
        warnings.append("Deeply nested each blocks - O(n²) complexity risk")
