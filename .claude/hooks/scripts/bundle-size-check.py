    ...
    if hook_event == "PostToolUse" and tool_name in ["Write", "Edit", "MultiEdit"]:
        ...
        try:
            ...
            if result.returncode != 0:
                print(
                    "Warning: Build failed, skipping bundle size check", file=sys.stderr
                )
                sys.exit(0)
            ...
            if baseline is not None:
                ...
                if increase_kb > MAX_INCREASE_KB:
                    ...
                    print(
                        f"ℹ️  Bundle size increased by {format_size(increase)} to {format_size(new_size)}",
                        file=sys.stderr,
                    )
                elif increase_kb < -10:
                    print(
                        f"✅ Bundle size decreased by {format_size(-increase)} to {format_size(new_size)}",
                        file=sys.stderr,
                    )
            else:
