...
def add_svelte_context(prompt):
    ...
    if re.search(r"\b(rune|runes|\$state|\$derived|\$effect)\b", prompt, re.I):
        context_parts.append(
            "Note: This project uses Svelte 5 with runes. Use $state(), $derived(), and $effect() syntax."
        )

    # Check for testing keywords
    if re.search(r"\b(test|testing|vitest|playwright|coverage)\b", prompt, re.I):
        context_parts.append(
            "Testing setup: Vitest for unit tests, Playwright for E2E tests. Run 'npm test' for unit tests."
        )

    # Check for Storybook keywords
    if re.search(r"\b(story|stories|storybook)\b", prompt, re.I):
        context_parts.append(
            "Storybook is configured with @storybook/sveltekit. Stories use Svelte CSF format."
        )

    # Check for performance keywords
    if re.search(r"\b(performance|optimize|bundle|speed|slow)\b", prompt, re.I):
        context_parts.append(
            "Performance tips: Use $state.raw() for large objects, implement virtual scrolling for long lists."
        )
...
def validate_prompt(prompt):
    ...
    sensitive_patterns = [
        (
            r'(?i)\b(password|secret|key|token|api[_-]?key)\s*[:=]\s*["\']?[\w-]+',
    ...
    for pattern, message in sensitive_patterns:
        if re.search(pattern, prompt):
            return (
                False,
                f"Security warning: {message}. Please remove sensitive information.",
            )
